...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    SELECT DISTINCT
      toysA.name, toysB.price 
    FROM 
      toys AS toysA
    JOIN 
      toys AS toysB
      ON toysA.name = toysB.name
    JOIN 
      toys AS toysC
      ON toysB.price = toysC.price 
    WHERE 
      toysC.name = 'Shiny Mouse'
    AND 
      toysA.name NOT LIKE '%Shiny%'
    ORDER BY 
      toysA.name ASC
  SQL
end
...
def just_like_orange
  ...
  execute(<<-SQL)
    SELECT 
      catsA.name, catsA.breed 
    FROM 
      cats AS catsA
    JOIN 
      cats AS catsB
      ON catsA.breed = catsB.breed 
    WHERE 
      catsB.name = 'Orange'
    AND 
      catsA.name NOT LIKE '%Orange%'
    ORDER BY 
      catsA.name ASC;
  SQL
end
